-- Rooms, part 3: how many a room holds.
--
-- The browser was already drawing "2/8" before this existed, which was a
-- promise the room could not keep - nothing stopped a ninth player, or a
-- ninetieth. This makes the number real.
--
-- A COLUMN rather than a constant, because the kinds are not the same size. A
-- singles room and a crew battle want different limits, and putting it on the
-- room means the browser can show each one's own rather than a number compiled
-- into the client. The default is 8 and every existing room gets it.

alter table pd_rooms
  add column if not exists capacity int not null default 8
    check (capacity between 2 and 32);

-- ------------------------------------------------------------------ create ---
--
-- Lets the caller pick, still defaulting to 8 so the existing call keeps
-- working unchanged.

-- ⚠️ The old four-argument version has to GO, not be replaced. Adding a fifth
-- parameter with a default makes an OVERLOAD, and then the existing call - four
-- named arguments - matches both of them and Postgres refuses it as ambiguous.
-- Dropping first is the difference between this migration working and room
-- creation breaking the moment it is applied.
drop function if exists pd_room_create(text, boolean, text, text);
...
begin
  ...
  insert into pd_rooms (code, mode, listed, passcode, owner, owner_name, capacity)
       values (v_code, p_mode, p_listed, v_pass, v_me, p_name,
               coalesce(p_capacity, 8));

  return json_build_object('ok', true, 'room', v_code, 'capacity',
                           coalesce(p_capacity, 8));
end $fn$;

grant execute on function pd_room_create(text, boolean, text, text, int)
  to authenticated;

-- -------------------------------------------------------------------- list ---
--
-- Carries the room's own capacity, so the browser shows what that room will
-- actually take rather than a number baked into the game.
...
-- -------------------------------------------------------------------- tick ---
--
-- Same function as part 2 with one guard added, restated in full because
-- Postgres replaces a function whole. The guard is the block marked "capacity".
